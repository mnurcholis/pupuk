...
class Laporan extends Component
{
    ...
    public function LaporanDatadiAgent()
    {
        $beli = TransaksiJualPagi::whereDoesntHave('transaksiJualSore')->orderby('tanggal', 'DESc')->get();

        if (count($beli) == 0) {
            session()->flash('datadiagent', 'Data Barang di agent tidak ada...');
            return;
        }
        ...
        $pdf = Pdf::loadView('laporan.databarangdiagent', $data)->output();

        $filename = 'data_barang_di_agent_report_' . now()->format('Ymd_His') . '.pdf';
